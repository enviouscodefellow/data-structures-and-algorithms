...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let filteredArr = arr.filter(obj => obj.mass > 77);
  return filteredArr.map(obj => obj.name).join(' - ');
};
...
const sortBy = (property, arr) => {
  ...
  if (typeof arr[0][property] === "number") {
    sortedArr = arr.sort((a, b) => a[property] - b[property]);
  } else if (typeof arr[0][property] === "string") {
    sortedArr = arr.sort((a, b) => a[property].localeCompare(b[property]));
  }
  return sortedArr;
};
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < flattenedArr.length; i++) {
    ...
    let winReqs = ["XXX", "OOO"];
    if (i === 0) {
      scan.push(flattenedArr[i] + flattenedArr[i + 1] + flattenedArr[i + 2]);
      scan.push(flattenedArr[i] + flattenedArr[i + 3] + flattenedArr[i + 6]);
      scan.push(flattenedArr[i] + flattenedArr[i + 4] + flattenedArr[i + 8]);
    }
    if (i === 1) scan.push(flattenedArr[i] + flattenedArr[i + 3] + flattenedArr[i + 6]);
    if (i === 2) {
      scan.push(flattenedArr[i] + flattenedArr[i + 2] + flattenedArr[i + 4]);
      scan.push(flattenedArr[i] + flattenedArr[i + 3] + flattenedArr[i + 6]);
    }
    if (i === 3 || i === 6) {
      scan.push(flattenedArr[i] + flattenedArr[i + 1] + flattenedArr[i + 2]); 
    }
    if (scan.some((element) => winReqs.includes(element))) winGame = true;
  }
  return winGame;
};
