...
function transformToLis(obj){
  ...
  for (let element in obj) {
    arr.push(`<li>${element}: ${obj[element]}</li>`);
  }
  return arr;
};
...
const count = (target, input) => {
  ...
  let total = flatArray.reduce((a, b) => {
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let characters = data.filter(object => object.gender === 'male' || object.gender === 'female').map(object=> object.name).reduce((a,b) => a + ' and ' + b, '');
  return characters.slice(5);
};
